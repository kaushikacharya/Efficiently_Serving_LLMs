...
def generate_next_token(model, tokenizer, batch):
    ...
    next_token_ids, past_key_values = generate_batch_tokens_with_past(model=model, inputs=inputs)
    next_tokens = tokenizer.batch_decode(next_token_ids)

    return get_next_inputs(batch=batch, next_token_ids=next_token_ids,
                            past_key_values=past_key_values, next_tokens=next_tokens)
...
def merge_batches(batch1, batch2):
    ...
    # KA: For each layer, pad key and value
    padded_kv1 = []
    ...
    attn_mask = torch.concat(
        [attn_mask1, attn_mask2], dim=0)
    
    # KA: For each layer, stack key, value of both the batches
    past_kv = []
...
def filter_batch(batch):
    ...
    remove_indices = []
    for i, token_remaining in enumerate(batch["tokens_remaining"]):
        if token_remaining <= 0:
            remove_indices.append(i)
    
    completed_responses = [
        r
        for i, r in enumerate(batch["responses"]) if i in remove_indices
    ]
...
def generate(model, tokenizer, requests):
    ...
    # run the initial prefill step
    batch = init_batch(tokenizer=tokenizer, requests=request_queue[:batch_size])
    cached_batch = generate_next_token(model=model, tokenizer=tokenizer, batch=batch)

    # update request queue
    request_queue = request_queue[batch_size:]

    # Continue until both
    # a) request queue is fully drained, and
    # b) every input within the cached_batch has completed generation
    while (len(request_queue) > 0 or cached_batch["input_ids"].size(0) > 0):
        ...
        if batch_capacity > 0 and len(request_queue) > 0:
            # prefill
            new_batch = init_batch(tokenizer=tokenizer, requests=request_queue[:batch_capacity])
            new_batch = generate_next_token(model=model, tokenizer=tokenizer, batch=new_batch)
        ...
        # decode
        cached_batch = generate_next_token(model=model, tokenizer=tokenizer, batch=cached_batch)

        # remove any inputs that have finished generation
        cached_batch, remove_indices, completed_responses = filter_batch(batch=cached_batch)

        for idx, resp in zip(remove_indices, completed_responses):
            responses[idx] = resp
